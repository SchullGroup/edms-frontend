      ...
      body: (
        <div>
          <div className="grid cols-2" style={{ gap: '12px' }}>
            ...
            <div className="field">
              <label>
                Client administrator <span className="req">*</span>
              </label>
              <input
                className="input"
          ...
          <div className="banner info">
            An isolated environment is created, usage metering begins immediately, and the
            administrator receives an invitation to complete branding and configuration.
          </div>
        </div>
      ),
      actions: [
        { label: 'Cancel' },
        {
          ...
          onClick: () => {
            ...
            const t = {
              ...
              domain:
                name
                  .trim()
                  .toLowerCase()
                  .replace(/[^a-z0-9]+/g, '') + '.schulltech.app',
              since: '2026-07',
              mrr: 0,
            };
            addTenant(t);
            auditAction(
              'TENANT_PROVISION',
              t.id,
              `Provisioned tenant ${t.name} (${model}, ${plan})`,
            );
            ...
            <button
              ...
              onClick={() => {
                ...
                openConfirm?.({
                  ...
                  message:
                    'Cross-tenant assist requires recorded client consent. A persistent banner is shown to all tenant users and every action is written to both audit logs.',
                  confirmLabel: 'Start with consent',
                  onConfirm: () => {
                    auditAction(
                      'ASSIST_START',
                      t.id,
                      'Started consented assist session for ' + t.name,
                    );
              ...
              <button
                ...
                onClick={() => {
                  ...
                  openConfirm?.({
                    ...
                    message:
                      'All tenant users lose access until reinstated. Data is retained per contract. This is reversible but disruptive.',
  ...
  return (
    <div>
      <div className="page-head">
        <div>
          ...
          <div className="page-sub">
            All tenants with status, plan, usage and health. Click a row for detail.
          </div>
        </div>
      ...
      <div className="grid cols-4 mb16">
        ...
        <div className="card kpi">
          <div className="kv">
            {tenants?.reduce((a: number, t: any) => a + t.users, 0).toLocaleString()}
          </div>
          <div className="kl">Total seats</div>
        </div>
        <div className="card kpi">
          <div className="kv">
            ${tenants?.reduce((a: number, t: any) => a + t.mrr, 0).toLocaleString()}
          </div>
