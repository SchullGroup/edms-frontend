...
export default function AuditorFindingsPage() {
      ...
      body: (
        <div>
          <div className="flex g8 wrap mb16">
            ...
            <span
              className={`badge ${f.status === 'Closed' ? 'b-status-closed' : f.status === 'Open' ? 'b-status-overdue' : 'b-status-pending'}`}
            >
              {f.status}
            </span>
            ...
            f.responses.map((r: any, i: number) => (
              ...
              >
                ...
                <div>
                  ...
                  >
                    {users?.find((u: any) => u.id === r.by)?.name} ·{' '}
                    {new Date(r.at).toLocaleString()}
                  </div>
                  <div className="body" style={{ fontSize: '13px' }}>
                    {r.text}
                  </div>
                </div>
              </div>
  ...
  const cols: Column<any>[] = [
    ...
    },
    {
      key: 'owner',
      label: 'Owner',
      render: (f) => users?.find((u: any) => u.id === f.owner)?.name || f.owner,
    },
    {
      key: 'due',
      label: 'Due',
      ...
      render: (f) => (
        <span
          style={
            f.status !== 'Closed' && f.due < Date.now()
              ? { color: 'var(--status-overdue)', fontWeight: 800 }
              : {}
          }
        >
      ...
      <div className="page-head">
        <div>
          ...
          <div className="page-sub">
            Raise findings, assign owners, track responses to closure with evidence.
          </div>
        </div>
