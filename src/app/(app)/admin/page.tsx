...
export default function AdminDashboard() {
  ...
  const pendingTasks = [
    { t: 'Approve workflow change request from D. Adeyemi (Invoice Approval v5 draft)', to: '/admin/workflows' },
    ...
    <div>
      ...
      <div className="grid cols-4 mb16">
        <div className="card kpi">
          <div className="kv">{users.length}</div>
      ...
      <div className="grid cols-2" style={{ alignItems: 'start' }}>
        <div className="card">
          <div className="card-head">
            ...
            <b className="tnum" style={{ color: pct === 100 ? 'var(--status-closed)' : 'var(--status-pending)' }}>{pct}%</b>
          </div>
          <div className="card-body">
            ...
            {setup.map((s, i) => (
              <div key={i} className="metric-li" style={{ cursor: 'pointer' }} onClick={() => router.push(s.to)}>
                <span className="flex aic g8">
                  <span style={{ color: s.done ? 'var(--status-closed)' : 'var(--status-pending)' }}>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ paddingTop: '6px' }}>
            {pendingTasks.map((p, i) => (
              <div key={i} className="metric-li" style={{ cursor: 'pointer' }} onClick={() => router.push(p.to)}>
