...
export default function CompliancePosturePage() {
  ...
  const { audit, findings, users } = useStore();
  ...
  const sensitiveActions = ['REDACT_RELEASE', 'REDACT_MARK', 'ACCESS_REQUEST', 'SIGN', 'PRINT', 'DOWNLOAD', 'SLA_ESCALATION', 'AUDIT_EXPORT'];
  const sens = audit?.filter((a: any) => sensitiveActions.includes(a.action)).slice(0, 8) || [];

  const hbarItems = [
    { label: 'Open', value: findings?.filter((f: any) => f.status === 'Open').length || 0, color: 'var(--status-overdue)' },
    { label: 'In Remediation', value: findings?.filter((f: any) => f.status === 'In Remediation').length || 0, color: 'var(--status-pending)' },
    { label: 'Closed', value: findings?.filter((f: any) => f.status === 'Closed').length || 0, color: 'var(--status-closed)' }
  ];
  ...
  const cols: Column<any>[] = [
    ...
    { key: 'due', label: 'Due', sortable: true, render: r => <span>{new Date(r.due).toLocaleDateString('en-GB')}</span> },
    { key: 'status', label: 'Status', render: r => <span className={`badge ${r.status === 'Open' ? 'b-status-overdue' : 'b-status-pending'}`}>{r.status}</span> },
  ];
    ...
    <div>
      ...
      <div className="grid cols-4 mb16">
        <div className="card kpi">
          <div className="kv" style={{ color: open.length ? 'var(--status-overdue)' : 'var(--status-closed)' }}>{open.length}</div>
      ...
      <div className="grid cols-2 mb16">
        <div className="card">
          <div className="card-head">
            ...
            <button className="btn btn-ghost btn-sm" onClick={() => router.push('/auditor/findings')}>Open tracker</button>
          </div>
            ...
            {sens.length > 0 ? (
              sens.map((a: any, idx: number) => (
                <div key={idx} className="meta-row">
                  <span className="k">{new Date(a.at).toLocaleString('en-GB', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>
                  <span className="v" style={{ textAlign: 'left', fontWeight: 500 }}>
                    {userById(users, a.user)?.name} · {a.action} — {a.detail}
                  </span>
                </div>
              ))
            ) : (
