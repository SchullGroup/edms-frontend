...
export default function TenantAuditPage() {
  ...
  const cols: Column<any>[] = [
    { key: 'at', label: 'When', sortable: true, render: (a) => new Date(a.at).toLocaleString() },
    {
      key: 'user',
      label: 'Actor',
      render: (a) => users.find((u: any) => u.id === a.user)?.name || a.user,
    },
    { key: 'action', label: 'Action', render: (a) => <span className="kbd">{a.action}</span> },
    {
      ...
      label: 'Target',
      render: (a) =>
        a.target?.startsWith('doc') ? (
          <a href={`#/doc/${a.target}`} onClick={(e) => e.stopPropagation()}>
    ...
    },
    {
      key: 'detail',
      label: 'Detail',
      render: (a) => <span style={{ fontSize: '12px' }}>{a.detail}</span>,
    },
  ];
