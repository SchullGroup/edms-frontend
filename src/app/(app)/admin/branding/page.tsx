...
export default function BrandingPage() {
  const { branding, updateBranding, auditAction } = useStore();
  const { setPageTitle, openModal, closeModal, addToast } = useUIStore();

  const [localB, setLocalB] = useState(branding || {
  ...
  const contrastRatio = (hex: string) => {
    const c = hex.replace('#', '');
    const [r, g, bl] = [0, 2, 4].map(i => parseInt(c.substr(i, 2), 16) / 255).map(v => v <= 0.03928 ? v / 12.92 : Math.pow((v + 0.055) / 1.055, 2.4));
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      document.documentElement.style.setProperty('--brand-primary-light', branding.primaryLight);
      document.documentElement.style.setProperty('--brand-accent', branding.accent);
    };
  }, [localB.primary, localB.primaryLight, localB.accent, branding]);

  const apply = () => {
    updateBranding(localB);
    auditAction('BRANDING_UPDATE', 'Branding', 'Updated tenant branding');
    addToast('Branding published — theme applied across the tenant', 'success');
  };
  ...
  const handleTemplateEdit = (t: string) => {
    openModal({
      ...
      body: (
        <div>
          <div style={{ border: '1px solid var(--border)', borderRadius: '9px', overflow: 'hidden' }}>
            <div style={{ background: localB.primary, color: '#fff', padding: '14px 18px', fontWeight: 800 }}>
              {localB.appName || 'EDMS'}
            </div>
            <div style={{ padding: '18px', fontSize: '13px', lineHeight: 1.6 }}>
              ...
              <p className="mt8">
                <span className="btn btn-sm" style={{ background: localB.accent, color: '#fff', pointerEvents: 'none' }}>
    ...
    <div>
      <div className="page-head">
        <div>
          ...
          <div className="page-sub">White-label theming — semantic tokens only; structure, hierarchy and accessibility stay fixed.</div>
        </div>
        <div className="actions">
          <button 
            className="btn btn-secondary" 
            onClick={() => {
              setLocalB({ ...localB, primary: '#1F3864', primaryLight: '#2E5496', accent: '#C55A11' });
              addToast('Reset to default palette (not yet published)', 'info');
            }}
          >
            Reset to default
          </button>
          <button className="btn btn-primary" onClick={apply}>Publish branding</button>
        </div>
      </div>
      ...
      <div className="grid cols-2" style={{ alignItems: 'start' }}>
        <div>
          <div className="card mb16">
            <div className="card-head"><span className="h3">Identity</span></div>
            <div className="card-body">
              ...
              <div className="field">
                ...
                />
                <div className="help">Production supports uploaded SVG/PNG with safe-area validation and light/dark variants.</div>
              </div>
              <div className="field">
            ...
            <div className="card-body">
              <div className="grid cols-3" style={{ gap: '12px' }}>
                <div className="field">
                  ...
                  <input 
                    ...
                    style={{ width: '46px', height: '34px', border: '1px solid var(--border)', borderRadius: '7px', cursor: 'pointer', padding: '2px' }}
              ...
              {badContrast.length > 0 ? (
                <div className="banner warning">
                  <span style={{ display: 'inline-flex', alignItems: 'center' }}><Icon name="alert" size={15} /></span>
                ...
                <div className="banner success">All brand colours pass WCAG AA contrast for white text.</div>
              )}
              ...
              <div className="flex g8 wrap">
                {PRESETS.map(p => (
                  <button 
                    ...
                    onClick={() => { 
                      setLocalB({ ...localB, primary: p.primary, primaryLight: p.light, accent: p.accent }); 
                      addToast(`Previewing “${p.name}” — publish to keep`, 'info'); 
                    }}
                  >
                    <span style={{ display: 'inline-flex', gap: '3px', marginRight: '6px' }}>
                      <span style={{ width: '10px', height: '10px', borderRadius: '3px', background: p.primary }}></span>
                      <span style={{ width: '10px', height: '10px', borderRadius: '3px', background: p.accent }}></span>
                    </span>
                    {p.name}
                  </button>
                ))}
        ...
        <div className="card">
          <div className="card-head"><span className="h3">Email templates</span></div>
          <div className="card-body">
            {['Task assigned', 'SLA reminder', 'Escalation', 'Signature request', 'Circular published', 'User invitation'].map(t => (
              <div key={t} className="metric-li">
                <span>{t}</span>
                <button className="btn btn-ghost btn-sm" onClick={() => handleTemplateEdit(t)}>Edit</button>
              </div>
            ))}
            <div className="divider"></div>
            <div className="h3 mb8">Login preview</div>
            <div style={{ border: '1px solid var(--border)', borderRadius: '10px', padding: '22px', textAlign: 'center', background: 'var(--surface)' }}>
              <div className="brand-logo" style={{ margin: '0 auto 10px', width: '40px', height: '40px', fontSize: '18px', background: localB.accent, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%' }}>
                {(localB.logoText || 'S').toUpperCase()}
              </div>
              <b>{localB.appName || 'SchullTech EDMS'}</b>
              <div className="caption">{localB.tenantName}</div>
              <div className="btn btn-sm mt8" style={{ background: localB.primary, color: '#fff', pointerEvents: 'none' }}>Sign in with SSO</div>
            </div>
          </div>
        </div>
      </div>
